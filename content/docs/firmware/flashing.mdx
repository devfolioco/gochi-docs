...
WSL2 is a lightweight VM with its own kernel, so USB serial devices need extra
plumbing that doesn't exist on a native Linux or macOS host. It works, but the
ESP32-C3's USB re-enumeration on reset makes it finicky.
...
| `Permission denied: /dev/ttyACM0` (WSL2)         | `dialout` doesn't apply — `sudo chmod 666 /dev/ttyACM0`. [WSL setup](#flashing-from-wsl2) |
| No `/dev/ttyACM*` in WSL2 after attaching        | Load the driver: `sudo modprobe cdc_acm`. [WSL setup](#flashing-from-wsl2) |
